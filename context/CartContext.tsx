...
import { v4 } from 'uuid';
import {
  useCreateDigitalPaymentOrder,
  useCreateManualPaymentOrder,
  useCreateStripePaymentIntent,
  useCreateStripePaymentOrder,
} from '@Lib/functions';
...
export const CartProvider = ({ businessId, children }: CartProviderProps) => {
  ...
  const [
    ...
    executingCreateManualPaymentOrder,
    errorCreateManualPaymentOrder,
  ] = useCreateManualPaymentOrder();
  ...
  const editLineWithId = (
    ...
    if (quantity == 0 || product == null || product == undefined) {
      // Remove from cart
      const newCart = lines.filter((cartLine) => {
        return cartLine.lineId != id;
      });
  ...
  const handleCreateStripePaymentIntent = (
    businessId: string,
    orderId: string
  ): Promise<string> => {
    return new Promise<string>(async (resolve, reject) => {
